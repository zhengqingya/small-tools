...
@Slf4j
@Service
public class MiniOmsOrderServiceImpl extends
        OmsOrderServiceImpl<OmsOrderMapper, OmsOrder> implements MiniOmsOrderService {
    ...
    @Resource
    private IUmsUserFeignApi umsUserFeignApi;
...
//        order.setUnPayEndTime(null);
    ...
    public void applyAfterSale(MiniOmsOrderApplyAfterSaleDTO params) {
        ...
        // 查询售后自动关闭时间
        ApiResult<SysPropertyVO> autoCloseAfterDayWrapper = this.ISysPropertyFeignApi.getByKey(
                SysPropertyKeyEnum.MALL_ORDER_SET_BUYER_APPLY_AFTER_SALE_HANDLE_MILLISECOND.getKey());
        autoCloseAfterDayWrapper.checkForRpc();
        long autoCloseAfterMillisecond = Long.parseLong(
                autoCloseAfterDayWrapper.getData().getValue());
