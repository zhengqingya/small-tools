...
public class OmsOrderServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<OmsOrderMapper, OmsOrder> implements OmsOrderService<OmsOrder> {
    ...
    public void confirmReceipt(MiniOmsOrderConfirmReceiptDTO params) {
        ...
        // 获取确认收货后-售后处理可申请处理时间
        ApiResult<SysPropertyVO> buyerApplyAfterSaleDayWrapper = this.ISysPropertyFeignApi.getByKey(
                SysPropertyKeyEnum.MALL_ORDER_SET_BUYER_APPLY_AFTER_SALE_MILLISECOND.getKey()
        );
        buyerApplyAfterSaleDayWrapper.checkForRpc();
        long buyerApplyAfterSaleMillisecond = Long.parseLong(buyerApplyAfterSaleDayWrapper.getData().getValue());
