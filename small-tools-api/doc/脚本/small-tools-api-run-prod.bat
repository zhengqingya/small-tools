...
set APP_NACOS_SERVER_ADDR=www.zhengqingya.com:8848
...
echo "run"
REM docker run -d -e PROFILE=%APP_PROFILE% -p %APP_PORT_GATEWAY%:%APP_PORT_GATEWAY% -p %APP_PORT_SYSTEM%:%APP_PORT_SYSTEM% -p %APP_PORT_BASIC%:%APP_PORT_BASIC% -p %APP_PORT_TOOL%:%APP_PORT_TOOL% --restart=always --name %APP_NAME% %APP_IMAGE%
REM pause
