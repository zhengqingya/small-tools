<template>
  <base-wraper>
    <base-header>
      <el-select v-model="listQuery.projectId" placeholder="项目" filterable style="width: 200px"
      ...
      <template #right>
        ...
        <el-button type="warning" icon="el-icon-search" @click="getVelocityContextList">
          参考模板数据源配置信息
        </el-button>
      </template>
    </base-header>
    ...
    <!-- 模板弹出框 -->
    <base-dialog v-model="dialogFormVisible" :title="textMap[dialogStatus]" top="50px" width="80%"
      @close="handleDialogClose">
      <el-form ref="dataForm" class="small-space" :model="form" label-position="left" label-width="120px" :rules="rules"
        size="small">
        <el-row :gutter="1">
          <el-col :span="12">
            <el-form-item label="归属项目:" prop="projectId">
              <el-select v-model="form.projectId" filterable placeholder="归属项目"
                @change="handleProjectChange(form.projectId)">
                <el-option v-for="item in projectList" :key="item.id" :value="item.id" :label="item.name" />
              </el-select>
            </el-form-item>
        ...
        <el-row :gutter="1">
          <el-col :span="11">
            <base-title-card title="模板内容：">
              <el-input v-model="form.content" style="height: 400px;overflow-y:auto;overflow-x:hidden;" type="textarea"
                :rows="20" placeholder="请输入内容" />
            </base-title-card>
          </el-col>
    ...
    <base-dialog v-model="dialogVelocityContextVisible" :title="textMap[dialogStatus]" @close="handleDialogClose">
      <base-table-p ref="projectVelocityContextTable" api="cg_project_template.listPageCodeProjectVelocityContext"
        :params="listQueryVelocityContext">
