    ...
    <el-card>
      <div slot="header">
        <el-row>
          <el-col :span="12">
            <base-table-cell>
              <base-cell-item label="表名" label-width="80px">{{
                  tableInfo.tableName
              }}</base-cell-item>
            </base-table-cell>
          </el-col>
      ...
      <div>
        <base-table-p v-loading="loading" height="700px" :index-code="true" :is-page="false"
          ...
          <el-table-column label="是否为主键" align="center">
            <template v-slot="scope">
              <el-select v-model="scope.row.ifPrimaryKey" style="width: 70px"
                :class="{ zq_select: scope.row.ifPrimaryKey }" disabled @change="handleChangePrimaryKey(scope.row)">
                <el-option v-for="item in typeList" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="是否自增长" align="center">
            <template v-slot="scope">
              <el-select v-model="scope.row.ifAutoIncrement" style="width: 70px"
                :class="{ zq_select: scope.row.ifAutoIncrement }" disabled>
                <el-option v-for="item in typeList" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </template>
          </el-table-column>
