<template>
  <base-wraper>
    <base-header>
      ...
      <template #right>
        <el-button type="warning" icon="el-icon-edit-outline" @click="handleUpdateColumnInfo()">更新表</el-button>
      </template>
    </base-header>
    ...
    <el-card>
      <div slot="header">
        <el-row>
          <el-col :span="12">
            <base-table-cell>
              <base-cell-item label="表名" label-width="80px">{{ tableInfo.tableName }}</base-cell-item>
            </base-table-cell>
          </el-col>
          <el-col :span="12">
            <base-table-cell>
              <base-cell-item label="表注释" label-width="80px">
      ...
      <div>
        <base-table-p v-loading="loading" height="700px" :index-code="true" :is-page="false" :data="tableInfo.columnInfoList">
          ...
          <el-table-column label="是否自增长" align="center">
            <template v-slot="scope">
              <el-select v-model="scope.row.ifAutoIncrement" style="width: 70px" :class="{ zq_select: scope.row.ifAutoIncrement }" disabled>
                <el-option v-for="item in typeList" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    async getColumnList() {
      this.loading = true
      let res = await this.$api.st_db_operate.getAllColumnsByDataSourceIdAndDbNameAndTableName(this.dataSourceId, this.dbName, this.tableName)
