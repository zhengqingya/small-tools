<template>
  <my-base-wraper>
    <base-header>
      ...
      <template #right>
        <el-button v-has="'add'" type="primary" @click="handleCreate">添加</el-button>
      </template>
    </base-header>
    ...
    <base-table-p ref="baseTable" api="st_crawler_website.listPage" :params="listQuery">
      ...
      <el-table-column align="center" label="操作" width="300">
        <template slot-scope="scope">
          <el-button v-has="'edit'" type="text" @click="handleUpdate(scope.row)">编辑</el-button>
          <my-base-delete-btn v-has="'delete'" @ok="handleDelete(scope.row.websiteId)" />
          <el-button type="text" @click="handleRefreshData(scope.row.url)">刷新爬虫数据</el-button>
        </template>
      </el-table-column>
    </base-table-p>

    <base-dialog :visible.sync="dialogFormVisible" :title="titleMap[dialogStatus]" width="30%" @close="handleDialogClose">
      ...
      <div slot="footer">
        ...
        <el-button v-has="'save'" type="primary" @click="submitForm">确 定</el-button>
      </div>
    </base-dialog>
  </my-base-wraper>
</template>
