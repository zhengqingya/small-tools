<template>
  <base-wraper>
    <base-header>
      <el-select v-model="listQuery.websiteId" placeholder="网站" style="width: 200px" clearable
      ...
      <el-input v-model="listQuery.title" placeholder="请输入文章标题" style="width: 200px" clearable
      ...
      <template #right>
        <base-upload-button v-if="showExportOrImportDataButton" api="st_crawler_article_info.importData"
          :params="listQuery" :show-file-list="false" @success="handleSuccess">
          <el-button type="danger">导入数据</el-button>
        </base-upload-button>
        <el-button v-if="showExportOrImportDataButton" type="success"
          @click="handleExportExcelData(listQuery.websiteId)">导出Excel</el-button>
        <el-button v-if="showExportOrImportDataButton" type="warning" @click="handleExportAllData(listQuery.websiteId)">
          导出所有数据</el-button>
      </template>
    </base-header>
    ...
    <base-dialog v-model="dialogFormVisible" :title="titleMap[dialogStatus]" width="50%">
      <el-form :model="form" label-width="100px">
        <base-table-cell>
          <base-cell-item label="ID">{{
              form.articleInfoId
          }}</base-cell-item>
        </base-table-cell>
        <base-table-cell>
          <base-cell-item label="数据来源网站">{{
              form.websiteName
          }}</base-cell-item>
        </base-table-cell>
        <base-table-cell>
          <base-cell-item label="分类">{{
              form.category
          }}</base-cell-item>
        </base-table-cell>
        <base-table-cell>
        ...
        <base-table-cell>
          <base-cell-item label="元数据URL"><a :href="form.url" target="_blank" style="color: #317bec">{{
              form.url
          }}</a></base-cell-item>
        </base-table-cell>
        <base-table-cell>
